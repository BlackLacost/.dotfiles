...
class App {
  ...
  App($argList, $pathIntrinsics) {
    ...
    $this._registerUserFolders = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders";
...
"@;
  ...
  configure() {
    ...
    $this._moveWindowDefaultDirectories();
    $this._ollamaLink();
    ...
    # Interactive.
    # $this._installFonts();
    $this._installApp("DEVCOM.JetBrainsMonoNerdFont");
  ...
  _moveWindowDefaultDirectories() {
    ...
    $this._moveWindowDefaultDirectory(
      ...
      $this._path(@("d:", "Downloads")),
      $downloadsGuid
    );
    # Restart explorer
    Stop-Process -ProcessName explorer -Force
  }
  _moveWindowDefaultDirectory([String] $oldPath, [String] $newPath, [String] $registerName) {
    ...
    Write-Host "Set $($this._registerUserFolders)\$registerName to $newPath";
    Set-ItemProperty -Path $this._registerUserFolders -Name $registerName -Value $newPath
  ...
  _installApp($appName) {
    ...
    & winget install --silent --accept-package-agreements $appName;
  ...
  _installAppWithVer($appName, $ver) {
    ...
    & winget install --silent --accept-package-agreements $appName --version $ver;
    if ($LASTEXITCODE -ne 0) { throw "Failed to install $appName" }
  }
